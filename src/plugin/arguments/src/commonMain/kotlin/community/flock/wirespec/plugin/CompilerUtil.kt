...
fun FilesContent.parse(logger: Logger): List<Pair<String, List<Node>>> = map { (name, source) ->
...
fun FilesContent.compile(logger: Logger, emitter: Emitter) = parse(logger)
...
fun Language.mapEmitter(packageName: PackageName, logger: Logger) = when (this) {
