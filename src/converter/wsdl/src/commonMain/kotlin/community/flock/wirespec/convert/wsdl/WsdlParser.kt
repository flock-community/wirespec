...
import community.flock.wirespec.convert.wsdl.WsdlParser.toType
...
object WsdlParser {
    ...
    fun parseDefinitions(xml: String, path: Path): AST {
        ...
        return importWsdl + importSchema + simpleTypes + complexTypes + elementTypes
    }
    ...
    fun SchemaBindings.Element.toField(): Field{
        ...
        return Field(
            ...
            reference = when (val t = type?.split(":")?.last()) {
                "decimal" -> Reference.Primitive(type = Reference.Primitive.Type.Number(Reference.Primitive.Type.Precision.P64), isIterable = isIterable)
                "string" -> Reference.Primitive(type = Reference.Primitive.Type.String, isIterable = isIterable)
                "integer" -> Reference.Primitive(type = Reference.Primitive.Type.Integer(Reference.Primitive.Type.Precision.P32), isIterable = isIterable)
