...
class JavaCompilationTest {
    ...
    fun testSimpleCompilation() {
        ...
        val handler = file("TodoHandler") {
            ...
            `interface`("TodoHandler", extends = type("Wirespec.Handler")) {
                asyncFunction("getTodo", type("Todo"))
            }
        }
    ...
    fun testComplexCompilation() {
        ...
        val response = file("Response") {
            `package`("com.example.endpoint")
            import("community.flock.wirespec.java.Wirespec")
            `interface`("Response", extends = type("Wirespec.Response", type("Object"))) {
                function("getStatus", integer) { returns(0) }
            }
        }
        ...
        val response200 = file("Response200") {
            `package`("com.example.endpoint")
            import("com.example.model.Todo")
            struct("Response200", interfaces = type("Response")) {
                field("body", list(type("Todo")))
                function("getStatus", integer) { returns(200) }
            }
        }
