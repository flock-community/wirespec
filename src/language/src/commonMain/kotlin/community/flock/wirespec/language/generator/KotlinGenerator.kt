...
private class KotlinEmitter(val file: File) {
    ...
    private fun Statement.emit(indent: Int): String = when (this) {
        ...
        is FunctionCall -> {
            val typeArgsStr =
                if (typeArguments.isNotEmpty() && name != "validate") "<${typeArguments.joinToString(", ") { it.emitGenerics() }}>" else ""
            val receiverStr = receiver?.let { "${it.emit()}." } ?: ""
            "$receiverStr${
    ...
    private fun Expression.emit(): String = when (this) {
        ...
        is FlatMapIndexed -> "${receiver.emit()}.flatMapIndexed { $indexVar, $elementVar -> ${body.emit()} }"
        is ListConcat -> when {
