...
import community.flock.wirespec.language.core.Constraint as LanguageConstraint
...
fun TypeWirespec.convertWithValidation(module: Module): File {
    ...
    return if (fieldValidations.isNotEmpty()) {
        file.transformMatchingElements { fn: community.flock.wirespec.language.core.Function ->
            if (fn.name == "validate") {
...
private fun FieldValidation.toExpression(): Expression {
    ...
    val valueRef: Expression = if (isNullable) VariableReference("it") else fieldRef
    ...
    val body: Expression = when (kind) {
        Kind.MODEL -> MapExpression(
            ...
            variable = "e",
            body = stringTemplate(text("$fieldPath."), expr(VariableReference("e"))),
        )
        ...
        Kind.MODEL_ARRAY -> FlatMapIndexed(
            ...
            body = MapExpression(
                receiver = FunctionCall(
                    receiver = VariableReference("el"),
                    name = "validate",
                    typeArguments = listOf(Type.Custom(typeName)),
                ),
                variable = "e",
                body = stringTemplate(text("$fieldPath["), expr(VariableReference("i")), text("]."), expr(VariableReference("e"))),
            ),
        )
        Kind.REFINED_ARRAY -> FlatMapIndexed(
            ...
            body = IfExpression(
                ...
                thenExpr = LiteralList(
                    listOf(stringTemplate(text("$fieldPath["), expr(VariableReference("i")), text("]"))),
                    Type.String,
                ),
                elseExpr = LiteralList(emptyList(), Type.String),
            ),
...
fun TypeWirespec.classifyValidatableFields(module: Module): List<FieldValidation> = buildList {
    for (field in shape.value) {
        val fieldName = field.identifier.sanitize()
...
fun RefinedWirespec.convert() = file(identifier.sanitize()) {
    struct(identifier.sanitize()) {
        ...
        function("validate") {
            ...
            returns(reference.convertConstraint(VariableReference("value")))
        }
    }
}
