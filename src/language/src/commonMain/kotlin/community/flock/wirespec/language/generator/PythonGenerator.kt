...
object PythonGenerator : CodeGenerator {
    ...
    private fun Expression.emit(): String = when (this) {
        ...
        is MapExpression -> "[${body.emit()} for $variable in ${receiver.emit()}]"
        is FlatMapIndexed -> "[item for $indexVar, $elementVar in enumerate(${receiver.emit()}) for item in ${body.emit()}]"
        is ListConcat -> when {
            lists.isEmpty() -> "[]"
    ...
    private fun Expression.emitWithInlinedIt(replacement: String): String = when (this) {
        ...
        is MapExpression -> "[${body.emitWithInlinedIt(replacement)} for $variable in ${receiver.emitWithInlinedIt(replacement)}]"
        is FlatMapIndexed -> "[item for $indexVar, $elementVar in enumerate(${receiver.emitWithInlinedIt(replacement)}) for item in ${body.emitWithInlinedIt(replacement)}]"
