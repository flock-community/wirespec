...
private class TypeScriptFileEmitter(val file: File) {
    ...
    private fun Statement.emit(indent: Int): String = when (this) {
        ...
        is AssertStatement -> "console.assert(${expression.emit()}, '$message');\n".indentCode(indent)
        is Switch -> {
    ...
    private fun Expression.emit(): String = when (this) {
        ...
        is MapExpression -> "${receiver.emit()}.map($variable => ${body.emit()})"
        is FlatMapIndexed -> "${receiver.emit()}.flatMap(($elementVar, $indexVar) => ${body.emit()})"
        is ListConcat -> when {
    ...
    private fun Expression.emitWithInlinedIt(replacement: String): String = when (this) {
        ...
        is MapExpression -> "${receiver.emitWithInlinedIt(replacement)}.map($variable => ${body.emitWithInlinedIt(replacement)})"
        is FlatMapIndexed -> "${receiver.emitWithInlinedIt(replacement)}.flatMap(($elementVar, $indexVar) => ${body.emitWithInlinedIt(replacement)})"
