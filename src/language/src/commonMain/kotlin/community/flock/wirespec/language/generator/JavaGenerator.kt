...
import community.flock.wirespec.language.core.AssertStatement
import community.flock.wirespec.language.core.Assignment
import community.flock.wirespec.language.core.BinaryOp
import community.flock.wirespec.language.core.Constraint
...
import community.flock.wirespec.language.core.FlatMapIndexed
import community.flock.wirespec.language.core.FunctionCall
import community.flock.wirespec.language.core.IfExpression
import community.flock.wirespec.language.core.Import
import community.flock.wirespec.language.core.Interface
import community.flock.wirespec.language.core.ListConcat
...
import community.flock.wirespec.language.core.MapExpression
import community.flock.wirespec.language.core.NotExpression
...
import community.flock.wirespec.language.core.StringTemplate
...
private class JavaEmitter(val file: File) {
    ...
    private fun Statement.emit(indent: Int, isInsideConstructor: Boolean = false): String = when (this) {
        ...
        is FunctionCall -> {
            val typeArgsStr = if (typeArguments.isNotEmpty() && name != "validate") "<${typeArguments.joinToString(", ") { it.emitGenerics() }}>" else ""
            val receiverStr = receiver?.let { "${it.emit()}." } ?: ""
        ...
        is BinaryOp -> when (operator) {
            BinaryOp.Operator.EQUALS -> "(${left.emit()}.equals(${right.emit()}));\n".indentCode(indent)
            BinaryOp.Operator.NOT_EQUALS -> "(!${left.emit()}.equals(${right.emit()}));\n".indentCode(indent)
    ...
    private fun Expression.emit(): String = when (this) {
        ...
        is MapExpression -> "${receiver.emit()}.stream().map($variable -> ${body.emit()}).toList()"
        is FlatMapIndexed -> {
            val recv = receiver.emit()
            val bodyWithSubstitution = body.emitWithSubstitution(elementVar, "$recv.get($indexVar)")
            "java.util.stream.IntStream.range(0, $recv.size()).mapToObj($indexVar -> $bodyWithSubstitution).flatMap(java.util.Collection::stream).toList()"
        }
        is ListConcat -> when {
    ...
    private fun Expression.emitWithSubstitution(varName: String, replacement: String): String = when (this) {
        ...
        is FunctionCall -> {
            ...
            "$receiverStr$typeArgsStr${name.sanitize()}(${args.joinToString(", ")})"
        }
        is FieldCall -> {
            val recv = receiver?.emitWithSubstitution(varName, replacement) ?: ""
            val dot = if (recv.isNotEmpty()) "." else ""
            "$recv$dot${field.sanitize()}()"
        }
        ...
        is MapExpression -> "${receiver.emitWithSubstitution(varName, replacement)}.stream().map($variable -> ${body.emitWithSubstitution(varName, replacement)}).toList()"
        is LiteralList -> {
            if (values.isEmpty()) {
