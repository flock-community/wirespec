...
open class KotlinEmitter(
    ...
    private fun Field.emitSerialized(fields: String) =
        """request.$fields.${identifier.emitVariableName()}?.let{"${identifier.emitVariableName()}" to serialization.serialize(it, typeOf<${reference.emit()}>())}"""
    ...
    private fun Field.emitDeserialized(fields: String) =
        if (isNullable)
            """${Spacer(3)}${identifier.emitVariableName()} = request.$fields["${identifier.emitVariableName()}"]?.let{ serialization.deserialize(it, typeOf<${reference.emit()}>()) }"""
        else
            """${Spacer(3)}${identifier.emitVariableName()} = serialization.deserialize(requireNotNull(request.$fields["${identifier.emitVariableName()}"]) { "${identifier.emitVariableName()} is null" }, typeOf<${reference.emit()}>())"""
