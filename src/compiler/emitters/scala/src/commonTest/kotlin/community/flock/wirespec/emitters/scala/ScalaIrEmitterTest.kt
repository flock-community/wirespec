...
class ScalaIrEmitterTest {
    ...
    fun compileFullEndpointTest() {
        val scala = """
            ...
            |      headers = Map("token" -> serialization.serializeParam[Token](request.headers.token, scala.reflect.classTag[Token]), "refreshToken" -> (request.headers.refreshToken.map(it => serialization.serializeParam[Token](it, scala.reflect.classTag[Token])).getOrElse(List.empty[String]))),
            ...
            |      refreshToken = (request.headers.get("refreshToken").map(it => serialization.deserializeParam[Token](it, scala.reflect.classTag[Token]))),
