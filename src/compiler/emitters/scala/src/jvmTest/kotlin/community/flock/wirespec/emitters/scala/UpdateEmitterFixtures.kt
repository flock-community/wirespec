...
private const val LANGUAGE = "scala"
...
private fun compile(fixture: Fixture): String = fixture.compiler { ScalaIrEmitter() }.fold(
    { error("Failed to compile fixture: $it") },
    { it },
)

private fun shared(): String = ScalaIrEmitter(emitShared = EmitShared(true)).emitShared()?.let(ScalaGenerator::generate) ?: error("Shared emit returned null")
...
fun main(args: Array<String>) {
    ...
    val fixtures: Map<String, () -> String> = linkedMapOf(
        "testEmitterType" to { emitNode(NodeFixtures.type) },
        "testEmitterEmptyType" to { emitNode(NodeFixtures.emptyType) },
        "testEmitterRefined" to { emitNode(NodeFixtures.refined) },
        "testEmitterEnum" to { emitNode(NodeFixtures.enum) },
