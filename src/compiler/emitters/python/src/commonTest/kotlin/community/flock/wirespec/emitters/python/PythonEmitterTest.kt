...
class PythonEmitterTest {
    ...
    fun compileFullEndpointTest() {
        val python = """
            ...
            |        queries = {
            |          "done": serialization.serialize_param(request.queries.done, bool),
            |          "name": serialization.serialize_param(request.queries.name, str)
            |        },
            |        headers = {
            |          "token": serialization.serialize_param(request.headers.token, Token),
            ...
            |          done = serialization.deserialize_param(request.queries.get("done"), bool),
            |          name = serialization.deserialize_param(request.queries.get("name"), str),
            |          token = serialization.deserialize_param(next((v for k, v in request.headers.items() if k.lower() == "token"), None), Token),
            |          RefreshToken = serialization.deserialize_param(next((v for k, v in request.headers.items() if k.lower() == "refresh-token"), None), Token),
            ...
            |            body = serialization.deserialize(response.body, TodoDto),
            |            token = serialization.deserialize_param(next((v for k, v in response.headers.items() if k.lower() == "token"), None), Token),
            |            refreshToken = serialization.deserialize_param(next((v for k, v in response.headers.items() if k.lower() == "refreshtoken"), None), Token)
            |          )
            |        case 500:
