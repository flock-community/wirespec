...
private const val LANGUAGE = "python"

private val emitContext = object : EmitContext, NoLogger {
    override val emitters = nonEmptySetOf(PythonIrEmitter())
}
...
private fun compile(fixture: Fixture): String = fixture.compiler { PythonIrEmitter() }.fold(
    { error("Failed to compile fixture: $it") },
    { it },
)

private fun shared(): String = PythonIrEmitter(emitShared = EmitShared(true)).emitShared()?.elements?.filterIsInstance<RawElement>()?.joinToString("") { it.code } ?: error("Shared emit returned null")
