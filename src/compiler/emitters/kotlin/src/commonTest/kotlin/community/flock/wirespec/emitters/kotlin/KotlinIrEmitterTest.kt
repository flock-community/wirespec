...
class KotlinIrEmitterTest {
    ...
    fun compileFullEndpointTest() {
        val kotlin = """
            ...
            |      headers = mapOf("token" to serialization.serializeParam<Token>(request.headers.token, typeOf<Token>()), "refreshToken" to (request.headers.refreshToken?.let { serialization.serializeParam<Token>(it, typeOf<Token>()) } ?: emptyList<String>())),
