...
interface TypeScriptClientEmitter: BaseEmitter, ClientEmitter, ParamEmitter, SpaceEmitter, ImportEmitter, TypeScriptTypeDefinitionEmitter {
    ...
    private fun Endpoint.Request.emitClientInterface(endpoint: Endpoint) =
        this.paramList(endpoint)
            .takeIf { it.isNotEmpty() }
            ?.joinToString(", ") { "${it.identifier.value}: ${it.reference.emit()}" }
            ?.let { "params: {${it}}" }
            .orEmpty()
    private fun emitFunction(endpoint: Endpoint, request: Endpoint.Request) = """
        |${endpoint.identifier.value}: async (${request.emitClientInterface(endpoint)}) => {
        |${Spacer}const req = ${endpoint.identifier.value}.request(${request.paramList(endpoint).takeIf { it.isNotEmpty() }?.let { "params" }.orEmpty()})
