...
class FileBuilder(private val name: String) : ContainerBuilder {
    ...
    fun main(block: FunctionBuilder.() -> Unit) {
        ...
        elements.add(Main(fn.body))
    }

    fun build(): File = File(Name.of(name), elements)
}

@Dsl
class NamespaceBuilder(private val name: String, private val extends: Type.Custom? = null) : ContainerBuilder {
    override val elements = mutableListOf<Element>()

    fun build(): Namespace = Namespace(Name.of(name), elements, extends)
}
...
class InterfaceBuilder(
    ...
    fun build(): Interface = Interface(Name.of(name), elements, extendsList, isSealed, typeParameters, fields)
}
...
class UnionBuilder(private val name: String, private val extends: Type.Custom? = null) : BaseBuilder {
    ...
    fun build(): Union = Union(Name.of(name), extends, members, typeParameters)
}
...
class EnumBuilder(private val name: String, private val extends: Type.Custom? = null) : ContainerBuilder {
    ...
    fun build(): Enum = Enum(Name.of(name), extends, entries, fields, constructors, elements)
}
...
class StructBuilder(private val name: String) : ContainerBuilder {
    ...
    fun build(): Struct = Struct(Name.of(name), fields, constructors, interfaces, elements)
}
