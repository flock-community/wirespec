...
class DslTest {
    ...
    fun testStaticElement() {
        val file = file("StaticModule") {
            namespace("MyService", extends = type("BaseService")) {
                struct("Config") {
                    field("url", string)
                }
                function("start") {
    ...
    fun testLiteralList() {
        val file = file("LiteralListModule") {
            function("test") {
                assign("tags", literalList(listOf(literal("tag1"), literal("tag2")), string))
                returns(RawExpression("tags"))
            }
        }
    ...
    fun testLiteralMap() {
        val file = file("LiteralMapModule") {
            function("test") {
                assign("scores", literalMap(mapOf("Alice" to literal(10), "Bob" to literal(20)), string, integer))
    ...
    fun testEmptyLiterals() {
        val file = file("EmptyLiteralsModule") {
            function("test") {
                assign("emptyList", literalList(string))
                assign("emptyMap", literalMap(string, integer))
        ...
        assertTrue(javaCode.contains("final var emptyList = java.util.List.<String>of();"))
        assertTrue(javaCode.contains("final var emptyMap = java.util.Collections.emptyMap();"))
        assertTrue(javaCode.contains("myFunc(java.util.List.<Integer>of(), java.util.Collections.emptyMap());"))
        assertTrue(javaCode.contains("new User(java.util.List.<String>of());"))

        assertTrue(tsCode.contains("const emptyList = [] as string[];"))
        assertTrue(tsCode.contains("const emptyMap = {};"))
        assertTrue(tsCode.contains("myFunc([] as number[], {});"))
        assertTrue(tsCode.contains("{ tags: [] as string[] };"))
    }
    ...
    fun testStructConstructor() {
        ...
        assertTrue(kotlinCode.contains("constructor(id: Int) : this(null, null) {"))
        assertTrue(kotlinCode.contains("this.id = id"))
        assertTrue(kotlinCode.contains("this.name = \"Anonymous\""))
    }
    ...
    fun testGenericsInExtends() {
        val file = file("GenericsExtendsModule") {
            ...
            namespace("Api", extends = type("BaseApi", integer)) {
                function("getData") {
                    returnType(string)
                }
            }
    ...
    fun testNullCheck() {
        ...
        val nullCheckExpr = NullCheck(
            expression = FieldCall(FieldCall(VariableReference(Name.of("request")), Name.of("queries")), Name.of("page")),
            body = FunctionCall(
                receiver = VariableReference(Name.of("serialization")),
                name = Name.of("serializeParam"),
                arguments = mapOf(
        ...
        assertTrue(javaCode.contains(".orElse(java.util.List.<String>of())"))

        // Kotlin: expression?.let { body } ?: alternative
        assertTrue(kotlinCode.contains("request.queries.page?.let { serialization.serializeParam(it) } ?: emptyList<String>()"))
        ...
        assertTrue(tsCode.contains(": [] as string[]"))
    ...
    fun testAssertStatement() {
        val file = file("AssertModule") {
            function("validate") {
                arg("value", integer)
                assertThat(
                    RawExpression("value > 0"),
        ...
        println("--- Java (Assert) ---")
        println(javaCode)
        println("--- Kotlin (Assert) ---")
        println(kotlinCode)
        println("--- TypeScript (Assert) ---")
        println(tsCode)
        println("--- Python (Assert) ---")
        println(pythonCode)

        // Java
        assertTrue(javaCode.contains("assert value > 0 : \"Value must be positive\";"))
    ...
    fun testDataObjectWithFields() {
        ...
        assertTrue(kotlinCode.contains("override val status: Int"))
        assertTrue(kotlinCode.contains("override val headers: Headers"))
        assertTrue(kotlinCode.contains("override val body: Unit"))
