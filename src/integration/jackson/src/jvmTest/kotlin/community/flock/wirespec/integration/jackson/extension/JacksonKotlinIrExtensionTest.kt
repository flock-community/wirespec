...
class JacksonKotlinIrExtensionTest {
    ...
    private fun jacksonEmitter(packageName: PackageName) = KotlinIrEmitter(packageName, EmitShared(false))
        .applyExtensions(listOf(JacksonSupportExtension(FileExtension.Kotlin)))
    ...
    fun `Should annotate unions with JsonTypeInfo and JsonSubTypes`() {
        ...
        assertContains(
            ...
            |@com.fasterxml.jackson.annotation.JsonSubTypes(value = [com.fasterxml.jackson.annotation.JsonSubTypes.Type(value = UserAccountPassword::class), com.fasterxml.jackson.annotation.JsonSubTypes.Type(value = UserAccountToken::class)])
    ...
    fun `Should leave records, refined types and enums untouched`() {
        ...
        for (fileName in listOf("Todo.kt", "TodoId.kt", "Color.kt", "UserAccountPassword.kt")) {
