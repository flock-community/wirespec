...
object AddPet : Wirespec.Endpoint {
  ...
  data class Request(
      ...
      constructor(body: Pet) : this(Path, Wirespec.Method.POST, Queries, RequestHeaders, body)
    }
  ...
  data class Response200(
      ...
      constructor(xRateLimit: Int?, body: Pet) : this(200, Response200Headers(xRateLimit = xRateLimit), body)
    }
  ...
  fun toRawRequest(serialization: Wirespec.Serializer, request: Request): Wirespec.RawRequest =
    Wirespec.RawRequest(
      ...
      body = serialization.serializeBody<Pet>(request.body, typeOf<Pet>())
    )
  fun fromRawRequest(serialization: Wirespec.Deserializer, request: Wirespec.RawRequest): Request =
    Request(body = (request.body?.let { serialization.deserializeBody<Pet>(it, typeOf<Pet>()) } ?: error("body is null")))
  fun toRawResponse(serialization: Wirespec.Serializer, response: Response<*>): Wirespec.RawResponse {
    when(val r = response) {
        is Response200 -> {
          return Wirespec.RawResponse(
            statusCode = r.status,
            headers = mapOf("X-Rate-Limit" to (r.headers.xRateLimit?.let { serialization.serializeParam<Int>(it, typeOf<Int>()) } ?: emptyList<String>())),
    ...
    when (response.statusCode) {
        200 -> {
          return Response200(
            ...
            body = (response.body?.let { serialization.deserializeBody<Pet>(it, typeOf<Pet>()) } ?: error("body is null"))
          )
        }
        405 -> {
          return Response405
