...
class SpringJavaEmitterTest {
    ...
    @Test
    fun shouldEmitTheFullWirespecAndAddAnnotationToTheHandlerMethod() {
        val path = Path("src/jvmTest/resources/todo.ws")
        val text = SystemFileSystem.source(path).buffered().readString()

        val ast = parse(text)
        val emitter = SpringJavaEmitter(PackageName("community.flock.wirespec.spring.test"))
        val actual = emitter.emit(ast, noLogger).map { it.result.trim() }.sorted()
