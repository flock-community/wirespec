...
- **transformerClasses**: List of fully qualified `IrTransformer` class names. The transformers are applied to the intermediate representation before code generation for every emitter that is an `IrEmitter`.
