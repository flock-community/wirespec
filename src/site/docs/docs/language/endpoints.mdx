...
```wirespec
endpoint GetTodos GET /todos # {`Authorization`: String, `Content-Type`: String, `X-Request-ID`: Integer} -> {
    200 -> Todo
}
```
...
The same backtick syntax can be used for response headers:

```wirespec
endpoint CreateTodo POST Todo /todos -> {
    201 -> Todo # {`Location`: String, `Content-Type`: String}
}
```
