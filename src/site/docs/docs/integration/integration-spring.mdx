...
Notice the defintion we specified earlier works on both sides, and the requests/responses are identical whether we are on
the producer on consumer side. This is central to Wirespec's design
