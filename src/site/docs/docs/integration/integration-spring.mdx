...
The integration is compatible with Spring Boot 3 and 4. It works with both Jackson 2 (the default on
Spring Boot 3) and Jackson 3 (the default on Spring Boot 4) for serialization and deserialization; the
right one is selected automatically based on what is on your classpath, with Jackson 3 taking precedence
...
<Tabs>
            ...
            <executions>
                <execution>
                    ...
                    <configuration>
                        ...
                        <extensions>
                            <extension>SpringMappingAnnotations</extension>
                        </extensions>
                    </configuration>
                </execution>
            </executions>
    ...
    <TabItem value="gradle" label="Gradle">
        Add the dependency to your dependencies and register a compile task. The Spring extensions
        are bundled with the Wirespec Gradle plugin, so no buildscript classpath configuration is needed.
        ...
        tasks.register<CompileWirespecTask>("wirespec-compile") {
            ...
            irExtensions = listOf(community.flock.wirespec.plugin.Extension.SpringMappingAnnotations)
        }
        ```
...
Wirespec's Spring integration includes built-in support for GraalVM native images. When you compile your Wirespec
definitions with the `SpringNativeHintsExtension` registered, a `WirespecNativeHints` class is automatically generated alongside your
models and endpoints. This class registers all generated types for reflection, which is required for serialization
and deserialization in a native image.

### How it works

The generated `WirespecNativeHints` class:

- Implements Spring's `RuntimeHintsRegistrar` interface
- Registers all generated model and endpoint classes (including inner classes) for reflection
- Is annotated with `@Configuration` and `@ImportRuntimeHints`, so Spring picks it up automatically

No additional configuration is required — the hints are generated and wired up as part of the normal compilation step.

Additionally, the `spring-jvm` library includes a `WirespecNativeConfiguration` that registers core Wirespec types
(`RawJsonBody`, `Wirespec.RawRequest`, `Wirespec.RawResponse`) for reflection, ensuring the framework internals also
work in a native image.

### Building a native image

To build a native image with Maven, add the `native-maven-plugin` and use the `native` profile:
...
[maven-spring-native](https://github.com/flock-community/wirespec/tree/master/examples/maven-spring-native)
example project.
:::
