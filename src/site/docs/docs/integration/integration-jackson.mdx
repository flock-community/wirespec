...
`JacksonExtension`, that annotates every generated union so the subtype is **deduced from the JSON
shape**:
...
:::note
`JacksonExtension` works for both the Java and Kotlin targets and runs as part of the IR pipeline.
