...
We want to deal with the `String` and then and then let the wrapper class deal with the validation (this keeps us
independent of Jackson's built-in validation).
...
This will give us the following record for the Name type:
...
This is great
However, if we would simply serialize the following Jackson would produce the following JSON:
