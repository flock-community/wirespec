    ...
    200 -> TodoDto
          ...
          <div id="generate">
            <section className={styles.generateSection}>
              <h2>Generate</h2>
              <p>The vision of code generation centers on enabling a contract–first approach where specifications act as the single source of truth. By prioritizing the specification over the implementation, development is streamlined, reducing ambiguity and fostering better collaboration among cross–functional teams. </p>
              <p>Interfaces, often mapped to the domain, play a pivotal role in this vision. They are designed to have a longer lifespan compared to implementation code and must remain isolated to ensure they are reusable, adaptable, and independent of specific technological constraints. This isolation allows interfaces to act as durable blueprints, transcending implementation changes and maintaining a stable foundation for the system’s evolution.</p>
              <p>Code generation helps bridge the gap between specification and implementation by automating the creation of typesafe, functional, and dependency–free code from the specification. This approach ensures that the generated code precisely reflects the defined interfaces, capturing all possible inputs, outputs, and interactions in a consistent and predictable manner.</p>
              <Tabs>
  ...
  └─ UpdateTodoEndpoint.java`}</CodeBlock>
          ...
          <div id="validate">
            <section className={styles.validateSection}>
              <h2>Validate</h2>
              <p>Specifications, such as those defined by Wirespec, play a critical role in validating the implementation of APIs by serving as the single source of truth. They ensure alignment between expected interfaces and actual code, facilitating robust development and testing practices.</p>
              <div className={styles.validateBlock}>
                <h3>Validation Through Specifications</h3>
                <p>Specifications provide a blueprint for the implementation. By describing the structure of data, endpoints, and channels in a concise, human–readable manner, they form the foundation for verifying that implementations align precisely with defined interfaces. This is especially helpful for cross–functional teams aiming to reduce ambiguity and miscommunication during development.</p>
              </div>
              <div className={styles.validateBlock}>
                <h3>Generating Random Test Data</h3>
                <p>Using specifications to generate randomized yet valid test data ensures that input data conforms to expected formats and constraints. Tools like Wirespec can leverage the detailed schema definitions to create test scenarios that systematically cover edge cases, identifying potential issues early in the development cycle. This automated data generation streamlines testing and improves reliability. </p>
              </div>
              <div className={styles.validateBlock}>
                <h3>Mock Servers for Testing</h3>
                <p>Mock servers can be generated based on the same specification, reducing dependency on live systems for testing. These servers simulate the behavior of APIs as defined in the contracts, allowing testing environments to mimic production without requiring complete backend implementations. This approach helps developers verify that their code interacts correctly with the API’s interface, ensuring input/output consistency and expected status codes are met. </p>
              </div>
              <div className={styles.validateBlock}>
                <h3>Detecting Discrepancies Early</h3>
                <p>By using the specification to drive both code generation and validation processes, inconsistencies can be detected before they reach production. Automated tests generated directly from the specification confirm that the code adheres to expected behaviors and data formats defined in the agreed–upon contract.</p>
              </div>
              <p>In summary, specifications like those enabled by Wirespec empower teams to validate implementations through code generation, automated testing, and mock servers, ensuring reliable and predictable API behavior. </p>
            </section>
          </div>
        </main>
      </div>
