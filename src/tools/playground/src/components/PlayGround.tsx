...
export function PlayGround({code, setCode, selectedLanguage, setSelectedLanguage}: PlayGroundProps) {
    ...
    useEffect(() => {
        if (wirespecOutput) {
            if (wirespecOutput.result.length) {
                setWirespecResult(wirespecOutput.result
                    .map(file => `${createFileHeaderFor(file.typeName, selectedLanguage)}${file.result}`)
                    .join(''));
            }
          ...
          <Grid item md={6}>
            <Box display="flex" alignItems="center" gap="5px">
              <Typography variant="h5">
                Wirespec
                <Box component="span" sx={{[theme.breakpoints.up('md')]: {display: 'none'}}}> to:</Box>
              </Typography>
              <Box sx={{[theme.breakpoints.up('md')]: {display: 'none'}}}>
                <TargetLanguageSelector selectedLanguage={selectedLanguage}
