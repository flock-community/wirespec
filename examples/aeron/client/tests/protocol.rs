use wirespec_aeron_client::gen::model::quote::Quote;
use wirespec_aeron_client::gen::model::quote_error::QuoteError;
use wirespec_aeron_client::gen::model::quote_list::QuoteList;
use wirespec_aeron_client::gen::model::watchlist::Watchlist;
use wirespec_aeron_client::gen::rpc::get_quote::GetQuote;
use wirespec_aeron_client::protocol::RpcFrame;
use wirespec_aeron_client::serialization::{
    get_quote_response, get_watchlist_quotes_response, json_to_quote, ping_response, watchlist_to_payload,
};

// These hex vectors are shared verbatim with the Kotlin integration
// (src/integration/aeron RpcFrameTest); a change here is a wire protocol change.
const GOLDEN_REQUEST: &str = "01012a00000000000000080047657451756f746509006165726f6e3a697063ea030000110000007b2273796d626f6c223a224141504c227d";
const GOLDEN_RESULT: &str = "01022a00000000000000080047657451756f74652e0000007b2273796d626f6c223a224141504c222c227072696365223a312e352c2263757272656e6379223a22555344227d";
const GOLDEN_ERROR: &str = "01032a00000000000000080047657451756f74652a0000007b22636f6465223a22554e4b4e4f574e5f53594d424f4c222c226d657373616765223a226e6f7065227d";

fn from_hex(hex: &str) -> Vec<u8> {
    (0..hex.len()).step_by(2).map(|i| u8::from_str_radix(&hex[i..i + 2], 16).unwrap()).collect()
}

fn to_hex(bytes: &[u8]) -> String {
    bytes.iter().map(|b| format!("{b:02x}")).collect()
}

#[test]
fn request_encodes_to_the_golden_wire_format() {
    let frame = RpcFrame::Request {
        correlation_id: 42,
        method: "GetQuote".into(),
        reply_channel: "aeron:ipc".into(),
        reply_stream_id: 1002,
        payload: br#"{"symbol":"AAPL"}"#.to_vec(),
    };
    assert_eq!(GOLDEN_REQUEST, to_hex(&frame.encode()));
}

#[test]
fn frames_round_trip() {
    for hex in [GOLDEN_REQUEST, GOLDEN_RESULT, GOLDEN_ERROR] {
        let frame = RpcFrame::decode(&from_hex(hex)).unwrap();
        assert_eq!(42, frame.correlation_id());
        assert_eq!(hex, to_hex(&frame.encode()));
    }
}

#[test]
fn result_frame_maps_to_the_generated_response_enum() {
    let frame = RpcFrame::decode(&from_hex(GOLDEN_RESULT)).unwrap();
    let expected = GetQuote::Response::Result(GetQuote::Result {
        value: Quote { symbol: "AAPL".into(), price: 1.5, currency: "USD".into() },
    });
    assert_eq!(expected, get_quote_response(&frame).unwrap());
}

#[test]
fn error_frame_maps_to_the_generated_response_enum() {
    let frame = RpcFrame::decode(&from_hex(GOLDEN_ERROR)).unwrap();
    let expected = GetQuote::Response::Error(GetQuote::Error {
        value: QuoteError { code: "UNKNOWN_SYMBOL".into(), message: "nope".into() },
    });
    assert_eq!(expected, get_quote_response(&frame).unwrap());
}

#[test]
fn plain_string_results_are_raw_utf8() {
    let frame = RpcFrame::Result { correlation_id: 1, method: "Ping".into(), payload: b"pong".to_vec() };
    assert_eq!("pong", ping_response(&frame).unwrap());
}

#[test]
fn quotes_validate_after_deserialization() {
    let quote = json_to_quote(&serde_json::json!({"symbol": "FLCK", "price": 42.0, "currency": "EUR"})).unwrap();
    assert!(quote.validate().is_empty());
}

#[test]
fn watchlists_serialize_as_a_json_object_keyed_by_field() {
    let watchlist = Watchlist { symbols: vec!["AAPL".into(), "FLCK".into()] };
    assert_eq!(br#"{"symbols":["AAPL","FLCK"]}"#.to_vec(), watchlist_to_payload(&watchlist));
}

#[test]
fn quote_list_frames_map_to_the_generated_model() {
    let frame = RpcFrame::Result {
        correlation_id: 7,
        method: "GetWatchlistQuotes".into(),
        payload: br#"{"quotes":[{"symbol":"AAPL","price":1.5,"currency":"USD"}]}"#.to_vec(),
    };
    let expected = QuoteList {
        quotes: vec![Quote { symbol: "AAPL".into(), price: 1.5, currency: "USD".into() }],
    };
    assert_eq!(expected, get_watchlist_quotes_response(&frame).unwrap());
}

#[test]
fn truncated_frames_are_rejected() {
    assert!(RpcFrame::decode(&from_hex(GOLDEN_REQUEST)[..20]).is_err());
    assert!(RpcFrame::decode(&[2, 1]).is_err());
}
