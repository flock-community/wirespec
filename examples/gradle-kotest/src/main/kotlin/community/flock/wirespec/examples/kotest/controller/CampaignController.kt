...
class CampaignController(private val service: CampaignService) :
    ...
    override suspend fun getCampaigns(request: GetCampaigns.Request): GetCampaigns.Response<*> =
        GetCampaigns.Response200(service.list(request.queries.status))

    override suspend fun getCampaign(request: GetCampaign.Request): GetCampaign.Response<*> =
        service.get(request.path.id)
            ?.let { GetCampaign.Response200(it) }
            ?: GetCampaign.Response404(Error(404, "Campaign ${request.path.id.value} not found"))

    override suspend fun createCampaign(request: CreateCampaign.Request): CreateCampaign.Response<*> =
        CreateCampaign.Response201(service.create(request.body))

    override suspend fun updateCampaign(request: UpdateCampaign.Request): UpdateCampaign.Response<*> =
        service.update(request.path.id, request.body)
            ?.let { UpdateCampaign.Response200(it) }
            ?: UpdateCampaign.Response404(Error(404, "Campaign ${request.path.id.value} not found"))

    override suspend fun activateCampaign(request: ActivateCampaign.Request): ActivateCampaign.Response<*> =
        service.activate(request.path.id)
            ?.let { ActivateCampaign.Response200(it) }
            ?: ActivateCampaign.Response404(Error(404, "Campaign ${request.path.id.value} not found"))

    override suspend fun deleteCampaign(request: DeleteCampaign.Request): DeleteCampaign.Response<*> =
        if (service.delete(request.path.id)) {
            DeleteCampaign.Response204
        } else {
            DeleteCampaign.Response404(Error(404, "Campaign ${request.path.id.value} not found"))
        }
}
