...
class LiveTodoRepository : TodoRepository {
    private val todos =
        mutableListOf(
            Todo(id = Todo.Id("8132b795-143f-4afb-8c8a-0608cb63c79c"), name = Name("Name"), done = true),
        )

    override fun getAllTodos(): List<Todo> = todos

    override fun getTodoById(id: Todo.Id): Todo = todos.find { it.id == id } ?: throw TodoNotFoundException(id = id)
