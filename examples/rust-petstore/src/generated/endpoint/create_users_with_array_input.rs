...
pub mod CreateUsersWithArrayInput {
    ...
    pub fn to_raw_request(serialization: &impl Serializer, request: Request) -> RawRequest {
        return RawRequest { method: format!("{:?}", request.method), path: vec![String::from("user"), String::from("createWithArray")], queries: std::collections::HashMap::new(), headers: std::collections::HashMap::new(), body: Some(serialization.serialize_body(&request.body, std::any::type_name::<Vec<User>>())) };
}
    pub fn from_raw_request(serialization: &impl Deserializer, request: RawRequest) -> Request {
        return Request::new(request.body.as_ref().map(|it| serialization.deserialize_body(&it, std::any::type_name::<Vec<User>>())).expect("body is null"));
}
