...
use regex;
use super::super::model::order::Order;
...
pub mod PlaceOrder {
    ...
    pub fn to_raw_request(serialization: &impl Serializer, request: Request) -> RawRequest {
        return RawRequest { method: format!("{:?}", request.method), path: vec![String::from("store"), String::from("order")], queries: std::collections::HashMap::new(), headers: std::collections::HashMap::new(), body: Some(serialization.serialize_body(&request.body, std::any::type_name::<Order>())) };
}
    pub fn from_raw_request(serialization: &impl Deserializer, request: RawRequest) -> Request {
        return Request::new(request.body.as_ref().map(|it| serialization.deserialize_body(&it, std::any::type_name::<Order>())).expect("body is null"));
}
    pub fn to_raw_response(serialization: &impl Serializer, response: Response) -> RawResponse {
        match response {
            Response::Response200(r) => {
                return RawResponse { status_code: r.status, headers: std::collections::HashMap::new(), body: Some(serialization.serialize_body(&r.body, std::any::type_name::<Order>())) };
}
    ...
    pub fn from_raw_response(serialization: &impl Deserializer, response: RawResponse) -> Response {
        match response.status_code {
            200_i32 => {
                return Response::Response200(Response200::new(response.body.as_ref().map(|it| serialization.deserialize_body(&it, std::any::type_name::<Order>())).expect("body is null")));
}
            400_i32 => {
    ...
    pub trait Handler {
        fn place_order(&self, request: Request) -> Response;
}
...
impl Server for Api {
    type Req = Request;
    type Res = Response;
    fn path_template(&self) -> &'static str { "/store/order" }
