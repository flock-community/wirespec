...
impl Request {
    pub fn new(username: String, body: User) -> Self {
        Request {
...
pub mod UpdateUser {
    ...
    pub fn to_raw_request(serialization: &impl Serializer, request: Request) -> RawRequest {
        return RawRequest { method: format!("{:?}", request.method), path: vec![String::from("user"), serialization.serialize_path(&request.path.username, std::any::type_name::<String>())], queries: std::collections::HashMap::new(), headers: std::collections::HashMap::new(), body: Some(serialization.serialize_body(&request.body, std::any::type_name::<User>())) };
}
    pub fn from_raw_request(serialization: &impl Deserializer, request: RawRequest) -> Request {
        return Request::new(serialization.deserialize_path(&request.path[1], std::any::type_name::<String>()), request.body.as_ref().map(|it| serialization.deserialize_body(&it, std::any::type_name::<User>())).expect("body is null"));
}
    ...
    pub trait Handler {
        fn update_user(&self, request: Request) -> Response;
}
