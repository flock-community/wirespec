...
pub mod FindPetsByStatus {
    ...
    pub fn to_raw_request(serialization: &impl Serializer, request: Request) -> RawRequest {
        return RawRequest { method: format!("{:?}", request.method), path: vec![String::from("pet"), String::from("findByStatus")], queries: std::collections::HashMap::from([(String::from("status"), serialization.serialize_param(&request.queries.status, std::any::type_name::<Vec<String>>()))]), headers: std::collections::HashMap::new(), body: None };
}
    pub fn from_raw_request(serialization: &impl Deserializer, request: RawRequest) -> Request {
        return Request::new(request.queries.get("status").as_ref().map(|it| serialization.deserialize_param(&it, std::any::type_name::<Vec<String>>())).expect("Param status cannot be null"));
}
