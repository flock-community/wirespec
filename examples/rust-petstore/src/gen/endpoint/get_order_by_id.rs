...
pub mod GetOrderById {
    use super::*;
    pub fn to_raw_request(serialization: &impl Serializer, request: Request) -> RawRequest {
        return RawRequest { method: format!("{:?}", request.method), path: vec![String::from("store"), String::from("order"), serialization.serialize_path(&request.path.order_id, std::any::TypeId::of::<i64>())], queries: std::collections::HashMap::new(), headers: std::collections::HashMap::new(), body: None };
    }
    pub fn from_raw_request(serialization: &impl Deserializer, request: RawRequest) -> Request {
        return Request::new(serialization.deserialize_path(&request.path[2], std::any::TypeId::of::<i64>()));
    }
    ...
    pub trait Handler {
        fn get_order_by_id(&self, request: Request) -> Response;
    }
