...
pub mod FindPetsByTags {
    ...
    pub fn to_raw_request(serialization: &impl Serializer, request: Request) -> RawRequest {
        return RawRequest { method: format!("{:?}", request.method), path: vec![String::from("pet"), String::from("findByTags")], queries: std::collections::HashMap::from([(String::from("tags"), serialization.serialize_param(&request.queries.tags, std::any::TypeId::of::<Vec<String>>()))]), headers: std::collections::HashMap::new(), body: None };
    }
    pub fn from_raw_request(serialization: &impl Deserializer, request: RawRequest) -> Request {
        return Request::new(request.queries.get("tags").as_ref().map(|it| serialization.deserialize_param(&it, std::any::TypeId::of::<Vec<String>>())).expect("Param tags cannot be null"));
    }
