...
pub mod LoginUser {
    ...
    pub fn to_raw_request(serialization: &impl Serializer, request: Request) -> RawRequest {
        return RawRequest { method: format!("{:?}", request.method), path: vec![String::from("user"), String::from("login")], queries: std::collections::HashMap::from([(String::from("username"), serialization.serialize_param(&request.queries.username, std::any::TypeId::of::<String>())), (String::from("password"), serialization.serialize_param(&request.queries.password, std::any::TypeId::of::<String>()))]), headers: std::collections::HashMap::new(), body: None };
    }
    pub fn from_raw_request(serialization: &impl Deserializer, request: RawRequest) -> Request {
        return Request::new(request.queries.get("username").as_ref().map(|it| serialization.deserialize_param(&it, std::any::TypeId::of::<String>())).expect("Param username cannot be null"), request.queries.get("password").as_ref().map(|it| serialization.deserialize_param(&it, std::any::TypeId::of::<String>())).expect("Param password cannot be null"));
    }
    pub fn to_raw_response(serialization: &impl Serializer, response: Response) -> RawResponse {
        match response {
            Response::Response200(r) => {
                return RawResponse { status_code: r.status, headers: std::collections::HashMap::from([(String::from("xExpiresAfter"), serialization.serialize_param(&r.headers.x_expires_after, std::any::TypeId::of::<String>())), (String::from("xRateLimit"), serialization.serialize_param(&r.headers.x_rate_limit, std::any::TypeId::of::<i32>()))]), body: Some(serialization.serialize_body(&r.body, std::any::TypeId::of::<String>())) };
            }
    ...
    pub fn from_raw_response(serialization: &impl Deserializer, response: RawResponse) -> Response {
        match response.status_code {
            200_i32 => {
                return Response::Response200(Response200::new(response.headers.get("xExpiresAfter").as_ref().map(|it| serialization.deserialize_param(&it, std::any::TypeId::of::<String>())).expect("Param xExpiresAfter cannot be null"), response.headers.get("xRateLimit").as_ref().map(|it| serialization.deserialize_param(&it, std::any::TypeId::of::<i32>())).expect("Param xRateLimit cannot be null"), response.body.as_ref().map(|it| serialization.deserialize_body(&it, std::any::TypeId::of::<String>())).expect("body is null")));
            }
            400_i32 => {
