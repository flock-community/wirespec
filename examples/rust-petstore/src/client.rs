...
async fn run(api: &impl PetstoreApi) {
    println!("=== Find pets by status 'available' ===");
    let resp = api.find_pets_by_status(find_pets_by_status::Request::new(vec!["available".into()])).await;
    match resp {
    ...
    println!("\n=== Get pet by ID ===");
    let resp = api.get_pet_by_id(get_pet_by_id::Request::new(1)).await;
    match resp {
    ...
    println!("\n=== Get store inventory ===");
    let resp = api.get_inventory(get_inventory::Request::new()).await;
    match resp {
    ...
    let resp = api.place_order(place_order::Request::new(order)).await;
    match resp {
