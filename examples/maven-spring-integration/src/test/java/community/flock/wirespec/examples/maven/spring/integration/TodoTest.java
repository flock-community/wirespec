...
class TodoTest {
    ...
    void shouldUploadAttachment() throws Exception {
        ...
        MockMultipartFile file = new MockMultipartFile("plain", "hello.txt", "text/plain", "Hello Wirespec".getBytes());

        MockMultipartFile csv = new MockMultipartFile(
                "csv", "hello.csv", "text/csv", "id,name,done\n1,'todo 1',true\n2,'todo 2',false".getBytes());
        ...
        MvcResult mvcMultipartResult = mockMvc.perform(multipart("/todos/{id}/upload", "1")
                        .file(file)
                        .file(json)
                        .file(csv)
        ...
        mockMvc.perform(asyncDispatch(mvcMultipartResult)).andExpect(status().isCreated());
