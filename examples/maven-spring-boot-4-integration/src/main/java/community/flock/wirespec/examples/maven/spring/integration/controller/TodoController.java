...
class TodoController
    ...
    public CompletableFuture<CreateTodo.Response<?>> createTodo(CreateTodo.Request request) {
        ...
        var todo = new Todo(UUID.randomUUID().toString(), todoInput.name(), todoInput.done());
        return service.create(todo).thenApply(CreateTodo.Response200::new);
    }

    @Override
    public CompletableFuture<DeleteTodo.Response<?>> deleteTodo(DeleteTodo.Request request) {
        return service.delete(request.path().id()).thenApply(DeleteTodo.Response200::new);
    }
