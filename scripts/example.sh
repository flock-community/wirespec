...
./gradlew src:integration:jackson:publishToMavenLocal --no-configuration-cache &&
./gradlew src:integration:spring:publishToMavenLocal --no-configuration-cache &&
# build examples
...
(cd "$dir"/../examples/spring-boot-integration && ./mvnw clean test)
